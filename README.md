...
![定位器界面预览](docs/locator-preview.png)

<p align="center"><i>↑ 在 QGIS 定位器里输入「匡河镇」的样子：两个数据源的结果分组列出，选中回车即定位</i></p>
...
| [二、功能一览](#二功能一览) | 能力清单 |
| [三、快速上手](#三快速上手) | 三步用起来 |
| [四、使用方法详解](#四使用方法详解) | 输入规则、落点、Key 配置 |
...
**打错字就搜不到。** 想找「匡河镇」，手一抖打成「框河镇」——主流地名接口都是纯字面匹配，直接给你 0 条结果，还不告诉你为什么。本插件会自动按同音字重试。

**只会读不会写。** 有些字（畈、塆、邾、郢）打得出来但不一定拼得对，可以直接输拼音 `sanlifan`、`kuanghezhen`。
...
| **同音字容错** | 「框河镇」→ 自动试出「匡河镇」；逐位轮转，词首词尾的错字都覆盖得到 |
| **拼音搜索** | 输 `kuanghezhen` 自动切分音节、转成汉字候选 |
| **视野内搜索** | 天地图源使用当前画布范围，级别由比例尺自动换算 |
| **分组展示** | 两个源的结果在定位器里分开成组，一眼看出哪条来自哪里 |
| **飞行定位** | 选中即飞到该位置，自动缩放到合适比例尺 |
| **结果落点** | 落在「地名搜索结果」内存图层，多次搜索累积，不写进你的数据 |
...
如果要用地名沿革，选中后插件会调详情接口；如果要搜 POI，加 `tdt` 前缀。
...
| **数据范围** | 标准地名库（法定地名） | 全量 POI（地名 + 机构 + 商铺） |
| **能不能搜到「匡河镇」** | 能，且只有 4 条相关 | 能，1033 条（含镇内每个单位） |
| **历史沿革 / 地名来历** | ✅ 独有 | ❌ |
| **按视野搜** | ❌ | ✅ |
| **需要 Key** | 不需要 | **需要** |
| **适合** | 查标准地名、查沿革 | 找 POI、找"这一带有什么" |
...
| `框河镇` | 先直搜，无结果则自动按同音字重试 |
| `kuanghezhen` | 识别为拼音，切分音节后转汉字候选 |
| `tdt 框河镇` | 天地图源也走同样的容错逻辑 |
...
1. 地图**飞到**该位置（自动处理 4326 → 你项目的坐标系）
2. 在 **`地名搜索结果`** 图层落一个点，字段：`name`（名称）、`ftype`（类别）、`note`（地址/地域）
3. 状态栏弹出提示

这个图层是**内存图层**，多次搜索会累积，关掉 QGIS 就没了——不会写进你的 GeoPackage 或数据库。想留档的话，右键导出即可。
...
这是本插件最有意思的部分。先说结论：**民政部和天地图都不支持同音字、也不支持拼音**。
...
flowchart TD
    ...
    C -->|"有"| Z["直接返回"]
    C -->|"没有"| D{"输入像拼音？"}
    D -->|"是（纯字母）"| E["切分音节<br/>kuang-he-zhen"]
    D -->|"否（中文）"| F["逐字查拼音<br/>框→kuang"]
...
| 日志内容 | 含义 |
|---|---|
| `民政部：「匡河镇」→ 4 条` | 正常，滤镜在工作 |
| `民政部：「框河镇」无结果，同音/拼音命中「匡河镇」` | 容错生效了 |
...
民政部是**标准地名库**（法定地名及其沿革），天地图是**全量 POI**（连镇上的财政所、林业站都算）。搜「匡河镇」前者 4 条、后者 1033 条，不是谁错了，是定位不同。
...
不能。它只处理**同音**（匡/框/筐）和**拼音输入**，不处理形近字（如「畈」写成「贩」读音不同就无能为力），也不纠正音调差异以外的语义错误。
...
- **同音容错不万能**——只换同音字，默认最多试 6 个候选。错两个字、或错的是生僻字，可能仍找不到。
- **拼音转汉字依赖字频**——极少见的字组合可能排在候选之外。
